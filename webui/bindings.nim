...
const
  WEBUI_VERSION* = "2.5.0-Beta.4" ## Version
  WEBUI_MAX_IDS* = (256)
  WEBUI_MAX_ARG* = (16)
...
proc newWindowId*(windowNumber: csize_t): csize_t {.webui,
    importc: "webui_new_window_id".}
...
proc `bind`*(window: csize_t; element: cstring; `func`: proc (
    e: ptr Event) {.cdecl.}): csize_t {.webui, importc: "webui_bind".}
  ##  Bind an HTML element and a JavaScript object with a backend function.
  ##  Empty `element` means all events.

proc setContext*(window: csize_t; element: cstring; context: pointer) {.webui,
    importc: "webui_set_context".}
  ##  Use this API after using `bind()` to add any user data to it that can be
  ##  read later using `getContext()`.

proc getContext*(e: ptr Event) {.webui,
    importc: "webui_get_context".}
  ##  Get user data that is set using `setContext()`.

proc getBestBrowser*(window: csize_t): csize_t {.webui,
    importc: "webui_get_best_browser".}
...
proc show*(window: csize_t; content: cstring): bool {.webui,
    importc: "webui_show".}
...
proc showClient*(e: ptr Event; content: cstring): bool {.webui,
    importc: "webui_show_client".}
  ##  Show a window using embedded HTML, or a file. If the window is already
  ##  open, it will be refreshed. Single client.

proc showBrowser*(window: csize_t; content: cstring;
    browser: csize_t): bool {.webui, importc: "webui_show_browser".}
  ##  Same as `show()`, but using a specific web browser.

proc startServer*(window: csize_t; path: cstring): cstring {.webui,
    importc: "webui_start_server".}
  ##  Same as `webui_show()`. But start only the web server and return the URL.
  ##  No window will be shown.

proc showWv*(window: csize_t; content: cstring): bool {.webui,
    importc: "webui_show_wv".}
...
proc setKiosk*(window: csize_t; status: bool) {.webui,
    importc: "webui_set_kiosk".}
  ##  Set the window in Kiosk mode (Full screen).

proc setCustomParameters*(window: csize_t; params: cstring) {.webui,
    importc: "webui_set_custom_parameters".}
  ##  Add a user-defined web browser's CLI parameters.

proc setHighContrast*(window: csize_t; status: bool) {.webui,
    importc: "webui_set_high_contrast".}
  ##  Setup the window with high-contrast support. Useful when you want to
  ##  build a better high-contrast theme with CSS.

proc setResizable*(window: csize_t; status: bool) {.webui,
    importc: "webui_set_resizable".}
...
proc browserExist*(browser: WebuiBrowser): bool {.webui,
    importc: "webui_browser_exist".}
...
proc setRootFolder*(window: csize_t; path: cstring): bool {.webui,
    importc: "webui_set_root_folder".}
  ##  Set the web-server root folder path for a specific window.

proc setBrowserFolder*(path: cstring) {.webui,
    importc: "webui_set_browser_folder".}
  ##  Set custom browser folder path.

proc setDefaultRootFolder*(path: cstring): bool {.webui,
    importc: "webui_default_set_root_folder".}
  ##  Set the web-server root folder path for all windows. Should be used
  ##  before `show()`.

proc setFileHandler*(window: csize_t; handler: proc (filename: cstring;
    length: ptr cint): pointer {.cdecl.}) {.webui,
...
proc setFileHandlerWindow*(window: csize_t; handler: proc (window: csize_t;
    filename: cstring; length: ptr cint): pointer {.cdecl.}) {.webui,

importc: "webui_set_file_handler_window".}
  ##  Set a custom handler to serve files. This custom handler should
  ##  return full HTTP header and body.
  ##  This deactivates any previous handler set with `webui_set_file_handler`

proc interfaceSetResponseFileHandler*(window: csize_t; response: pointer;
    length: cint) {.webui,
    importc: "webui_interface_set_response_file_handler".}
...
proc setIcon*(window: csize_t; icon: cstring; `type`: cstring) {.webui,
...
proc memcpy*(dest: pointer; src: pointer; count: csize_t) {.webui,
    importc: "webui_memcpy".}
  ##  Copy raw data.

proc sendRaw*(window: csize_t; function: cstring; raw: pointer;
    size: csize_t) {.webui, importc: "webui_send_raw".}
  ##  Safely send raw data to the UI. All clients.

proc sendRawClient*(e: ptr Event; function: cstring; raw: pointer;
    size: csize_t) {.webui, importc: "webui_send_raw_client".}
  ##  Safely send raw data to the UI. Single client.

proc setHide*(window: csize_t; status: bool) {.webui,
    importc: "webui_set_hide".}
  ##  Set a window in hidden mode. Should be called before `show()`.

proc setSize*(window: csize_t; width: cuint; height: cuint) {.webui,
    importc: "webui_set_size".}
  ##  Set the window size.

proc setMinimumSize*(window: csize_t; width: cuint; height: cuint) {.webui,
    importc: "webui_set_minimum_size".}
  ##  Set the window minimum size.

proc setPosition*(window: csize_t; x: cuint; y: cuint) {.webui,
    importc: "webui_set_position".}
  ##  Set the window position.

proc setCenter*(window: csize_t) {.webui, importc: "webui_set_center".}
  ##  Centers the window on the screen. Works better with
  ##  WebView. Call this function before `show()` for better results.

proc setProfile*(window: csize_t; name: cstring; path: cstring) {.webui,
    importc: "webui_set_profile".}
  ##  Set the web browser profile to use. An empty `name` and `path` means
  ##  the default user profile. Must be called before `show()`.

proc setProxy*(window: csize_t; proxy_server: cstring) {.webui,
    importc: "webui_set_proxy".}
...
proc setPublic*(window: csize_t; status: bool) {.webui,
    importc: "webui_set_public".}
  ##  Allow a specific window address to be accessible from a public network.

proc navigate*(window: csize_t; url: cstring) {.webui,
    importc: "webui_navigate".}
  ##  Navigate to a specific URL. All clients.

proc navigateClient*(e: ptr Event; url: cstring) {.webui,
    importc: "webui_navigate_client".}
...
proc getParentProcessId*(window: csize_t): csize_t {.webui,
    importc: "webui_get_parent_process_id".}
  ##  Get the ID of the parent process (The web browser may re-create
  ##  another new process).

proc getChildProcessId*(window: csize_t): csize_t {.webui,
    importc: "webui_get_child_process_id".}
  ##  Get the ID of the last child process.

proc win32GetHwnd*(window: csize_t): pointer {.webui,
    importc: "webui_win32_get_hwnd".}
...
proc setPort*(window: csize_t; port: csize_t): bool {.webui,
    importc: "webui_set_port", discardable.}
...
proc setConfig*(option: WebuiConfig; status: bool) {.webui,
    importc: "webui_set_config".}
  ##  Control WebUI's behaviour. It's recommended to call this at the beginning.

proc setEventBlocking*(window: csize_t; status: bool) {.webui,
    importc: "webui_set_event_blocking".}
...
proc setFrameless*(window: csize_t; status: bool) {.webui,
    importc: "webui_set_frameless".}
  ##  Make a WebView window frameless.

proc setTransparent*(window: csize_t; status: bool) {.webui,
    importc: "webui_set_transparent".}
  ##  Make a WebView window transparent.

proc getMimeType*(file: cstring): cstring {.webui,
    importc: "webui_get_mime_type".}
  ##  Get the HTTP mime type of a file.

# -- SSL/TLS -------------------------

proc setTlsCertificate*(certificate_pem: cstring;
    private_key_pem: cstring): bool {.webui,
...
proc runClient*(e: ptr Event; script: cstring) {.webui,
    importc: "webui_run_client".}
  ##  Run JavaScript without waiting for the response. Single client.

proc script*(window: csize_t; script: cstring; timeout: csize_t;
    buffer: cstring; bufferLength: csize_t): bool {.webui,
...
proc scriptClient*(e: ptr Event; script: cstring; timeout: csize_t;
    buffer: cstring; bufferLength: csize_t): bool {.webui,
    importc: "webui_script_client".}
  ##  Run JavaScript and get the response back. Make sure your local buffer can
  ##  hold the response. Single client.

proc setRuntime*(window: csize_t; runtime: csize_t) {.webui,
...
proc getIntAt*(e: ptr Event; index: csize_t): clonglong {.webui,
    importc: "webui_get_int_at".}
  ##  Get an argument as integer at a specific index.

proc getInt*(e: ptr Event): clonglong {.webui, importc: "webui_get_int".}
  ##  Get the first argument as integer.

proc getFloatAt*(e: ptr Event; index: csize_t): cdouble {.webui,
    importc: "webui_get_float_at".}
...
proc getStringAt*(e: ptr Event; index: csize_t): cstring {.webui,
    importc: "webui_get_string_at".}
  ##  Get an argument as string at a specific index.

proc getString*(e: ptr Event): cstring {.webui, importc: "webui_get_string".}
  ##  Get the first argument as string.

proc getBoolAt*(e: ptr Event; index: csize_t): csize_t {.webui,
    importc: "webui_get_bool_at".}
  ##  Get an argument as boolean at a specific index.

proc getBool*(e: ptr Event): csize_t {.webui, importc: "webui_get_bool".}
  ##  Get the first argument as boolean.

proc getSizeAt*(e: ptr Event; index: csize_t): csize_t {.webui,
    importc: "webui_get_size_at".}
  ##  Get the size in bytes of an argument at a specific index.

proc getSize*(e: ptr Event): csize_t {.webui, importc: "webui_get_size".}
  ##  Get size in bytes of the first argument.

proc returnInt*(e: ptr Event; n: clonglong) {.webui,
    importc: "webui_return_int".}
  ##  Return the response to JavaScript as integer.

proc returnFloat*(e: ptr Event; f: cdouble) {.webui,
    importc: "webui_return_float".}
  ##  Return the response to JavaScript as integer.

proc returnString*(e: ptr Event; s: cstring) {.webui,
    importc: "webui_return_string".}
  ##  Return the response to JavaScript as string.

proc returnBool*(e: ptr Event; b: bool) {.webui, importc: "webui_return_bool".}
  ##  Return the response to JavaScript as boolean.

proc getLastErrorNumber*(): csize_t {.webui,
    importc: "webui_get_last_error_number".}
  ##  Get the last WebUI error code.

proc getLastErrorMessage*(): cstring {.webui,
    importc: "webui_get_last_error_message".}
  ##  Get the last WebUI error code.

#  -- Interface -----------------------

proc interfaceBind*(window: csize_t; element: cstring; `func`: proc (
    window: csize_t; eventType: csize_t; element: cstring;
        eventNumber: csize_t;
    bindId: csize_t) {.cdecl.}): csize_t {.webui,
...
proc interfaceSetResponse*(window: csize_t; event_number: csize_t;
    repsonse: cstring) {.webui, importc: "webui_interface_set_response".}
  ##  When using `interfaceBind()`, you may need this function to easily set a
  ##  response.

proc interfaceIsAppRunning*(): bool {.webui,
    importc: "webui_interface_is_app_running".}
  ##  Check if the app still running or not. This replaces `wait()`.

proc interfaceGetWindowId*(window: csize_t): csize_t {.webui,
    importc: "webui_interface_get_window_id".}
  ##  Get a unique window ID.

proc interfaceGetIntAt*(window: csize_t; event_number: csize_t;
    index: csize_t): clonglong {.webui, importc: "webui_interface_get_int_at".}
  ##  Get an argument as an integer at a specific index.

proc interfaceGetFloatAt*(window: csize_t; event_number: csize_t;
    index: csize_t): cdouble {.webui, importc: "webui_interface_get_float_at".}
  ##  Get an argument as a float at a specific index.

proc interfaceGetStringAt*(window: csize_t; event_number: csize_t;
    index: csize_t): cstring {.webui, importc: "webui_interface_get_string_at".}
  ##  Get an argument as a string at a specific index.

proc interfaceGetBoolAt*(window: csize_t; event_number: csize_t;
    index: csize_t): bool {.webui, importc: "webui_interface_get_bool_at".}
  ##  Get an argument as a boolean at a specific index.

proc interfaceGetSizeAt*(window: csize_t; event_number: csize_t;
    index: csize_t): csize_t {.webui, importc: "webui_interface_get_size_at".}
  ##  Get the size in bytes of an argument at a specific index.
